...
const anagram = (str1, str2)=>{
    let first = helper(str1);
    let second = helper(str2);
    for(let key in first){
      if(first[key] !== second[key]){// 1 !== 1
        return false;
      }
    }
    if(Object.keys(first).length !== Object.keys(second).length){//array.length !== array.length
      return false;
    }
    return true;
